...
cron.schedule('0 0 1 * *', function() { // first day of every month
  new ClientDataSaver().emailOverview();
});
...
class Emailer {
  ...
  async doSendEmail(to, from, userId, html) {
    const messageParts = [
      'From: ' + from,
      'To: ' + to,
  ...
  async sendEmail(to, from, userId, html) {
    // this.doSendEmail(to, from, userId, html)
    console.log("Email would have been sent to: " + to + ', from: ' + from + ', starting with: ' + html.substring(0, 19))
  }
}
...
class ClientDataSaver {
  ...
  async getOverview() {
    let s = 'timestamp,provider,zip code,race,age range,education level,employment status,disabilities,criminal history,legal resident,english lang,id<br/>'
  ...
  emailOverview() {
    this.overview(function resolve(data) {
      for (let e of ['chris.keith@gmail.com']) { // more email addresses to come
        emailer.sendEmail('workforce.midsouth@gmail.com', e, 
                          'workforce.midsouth@gmail.com', data)
      }
    })
  }
}
